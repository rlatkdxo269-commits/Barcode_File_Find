...
프로그램은 실행 시 GitHub 최신 릴리스를 확인합니다.

현재 설치된 버전보다 높은 릴리스가 있으면 `Barcode_File_Find_Setup.exe`를 자동으로 내려받아 조용한 설치(`/Q`)로 업데이트를 시작하고, 실행 중인 프로그램은 종료됩니다. 업데이트 설치가 끝난 뒤 시작 메뉴에서 다시 실행하면 새 버전이 적용됩니다.

새 버전을 배포할 때는 다음 순서를 사용합니다.

1. `Barcode_File_Find.csproj`의 `Version`, `AssemblyVersion`, `FileVersion`, `InformationalVersion` 값을 올립니다.
2. 설치 파일을 다시 빌드합니다.
3. GitHub Release 태그를 새 버전으로 생성합니다. 예: `v2.0.0`
4. 해당 릴리스 자산으로 `Barcode_File_Find_Setup.exe`를 업로드합니다.

기존 사용자는 다음 실행 시 새 릴리스를 감지하고 자동 업데이트됩니다.
